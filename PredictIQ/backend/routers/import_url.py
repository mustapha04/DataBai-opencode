...
async def import_from_url(req: ImportUrlRequest):
    ...
    elif "tsv" in content_type or "tab" in content_type:
        filename = "import.tsv"

    try:
        df, meta = parse_file(content, filename)
    ...
    row_count = len(df)
    col_count = len(meta)
    safe_name = f"url-{req.url.split('/')[-1][:40] or 'import'}.csv"
    ...
    try:
        ...
        if products:
            ...
            for i in range(0, len(products), batch_size):
                supabase.table("products").insert(products[i:i + batch_size]).execute()
        ...
        rows_json = df.fillna("").astype(str).to_dict(orient="records")
        return {
            "id": "local-" + str(uuid.uuid4()),
            "file_name": safe_name,
            "row_count": row_count,
            "column_count": col_count,
            "data_quality_score": quality,
