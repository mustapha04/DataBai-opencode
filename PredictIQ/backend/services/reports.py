...
def _sanitize(text: str) -> str:
    """Replace characters not supported by the built-in Helvetica font."""
    replacements = {
        '\u2013': '-', '\u2014': '--', '\u2018': "'", '\u2019': "'",
        '\u201c': '"', '\u201d': '"', '\u2026': '...', '\u00a0': ' ',
        '\u2022': '*', '\u2010': '-', '\u2011': '-', '\u2012': '-',
...
def generate_pdf_report(
    ...
    # Normalize meta — may be a list of dicts or a {types, columns} object
    ...
    num_cols = [c["name"] for c in meta_list if c.get("type") == "numeric"]
    cat_cols = [c["name"] for c in meta_list if c.get("type") == "categorical"]
    ...
    pdf.cell(0, 7, _sanitize(file_name), align="C", new_x="LMARGIN", new_y="NEXT")
    ...
    summary_lines.append(f"Data Quality Score: {_calc_quality(meta_list):.0f}%")
    ...
    if revenue_col and cat_col:
        pdf.section_title(f"Revenue by {_sanitize(cat_col)}")
    ...
    if revenue_col:
        ...
        for _, row in top.iterrows():
            ...
            for i, v in enumerate(vals):
                pdf.cell(col_widths[i], 5.5, _sanitize(v), border=1, align="C")
            pdf.ln()
...
def _calc_quality(meta_list: list[dict]) -> float:
    total = sum(c.get("total", c.get("null_count", 1)) for c in meta_list) or 1
    nulls = sum(c.get("nulls", c.get("null_count", 0)) for c in meta_list)
