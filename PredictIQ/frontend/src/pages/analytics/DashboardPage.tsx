            ...
            <div style={{ position: 'relative' }}>
              ...
              {showExport && (
                <div style={{
                  ...
                  <div style={{ padding: '8px 14px', cursor: 'pointer', fontSize: 13, color: 'var(--text)' }}
                    onClick={() => { setShowExport(false); toast('Exporting CSV...', 'info'); exportData(id!, 'csv', data); }}>
                    Export as CSV
                  </div>
                </div>
              )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 24 }}>
        {kpis.map(kpi => (
          <div key={kpi.label} className="card" style={{ position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>
              {kpi.label}
            </div>
            <div style={{ fontSize: 24, fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{kpi.value}</div>
            <div style={{ fontSize: 12, color: 'var(--dim)', marginTop: 2 }}>{kpi.sub}</div>
          </div>
        ))}
      </div>
...
async function exportData(datasetId: string, format: string, data: LocalData | null) {
  const token = localStorage.getItem('predictiq-token');
  const base = import.meta.env.VITE_API_URL || '';
  const isLocal = datasetId.startsWith('local-');
