...
export default function UploadPage() {
  ...
  const handleUrlImport = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { toast(data.detail || 'Import failed', 'error'); setUrlLoading(false); return; }

      if (data.is_local && data.rows) {
        sessionStorage.setItem(data.id, JSON.stringify({
          rows: data.rows, meta: data.columns_meta, fileName: data.file_name,
        }));
        setParseResult({
          rows: data.rows, meta: data.columns_meta, fileName: data.file_name,
          quality: data.data_quality_score, datasetId: data.id, isLocal: true,
        });
        setStage('quality');
      } else {
        toast('Dataset imported from URL', 'success');
        navigate(`/dashboard/${data.id}`);
      }
    } catch (err) {
      console.error(err);
      toast('Failed to import URL', 'error');
    } finally {
      setUrlLoading(false);
    }
  }, [urlInput, navigate]);
  ...
  const handleClean = useCallback(async () => {
    ...
    try {
      if (parseResult.isLocal) {
        ...
        navigate(`/dashboard/${parseResult.datasetId}`);
      } else {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '48px auto' }}>
      ...
      <div style={{ marginTop: 20, borderTop: '1px solid var(--border)', paddingTop: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, textAlign: 'center' }}>Or import from a URL</div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          />
          <button className="btn btn-primary" onClick={handleUrlImport} disabled={loading || !urlInput.trim()}
